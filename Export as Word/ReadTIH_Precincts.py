    ...
    if response.status_code == 200:
        ...
        if not data['data']:
            ...
            for item in data['data']:
                # Get the "type" of the precinct
                precinct_type = item.get("type", "Unknown")

                # Create a list for the type if it doesn't exist in the grouped_data dictionary
                if precinct_type not in grouped_data:
                    grouped_data[precinct_type] = []
                ...
                grouped_data[precinct_type].append(item)
...
for precinct_type, type_items in grouped_data.items():
    # Add a section header for the precinct type
    doc.add_heading(f"Precint Type: {precinct_type}", level=1)

    # Iterate through the precincts of the current type
    for item in type_items:
        ...
        doc.add_paragraph(f"Recommended Dwell Time: {item['recommendedDwellTime']}")
...
doc.save('precincts_grouped.docx')

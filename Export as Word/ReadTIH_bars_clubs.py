    ...
    if response.status_code == 200:
        ...
        if not data['data']:
            ...
            for item in data['data']:
                # Get the "type" of the bars_club
                bars_club_type = item.get("type", "Unknown")

                # Create a list for the type if it doesn't exist in the grouped_data dictionary
                if bars_club_type not in grouped_data:
                    grouped_data[bars_club_type] = []

                # Append the current item to the list for its type
                grouped_data[bars_club_type].append(item)
...
for bars_club_type, type_items in grouped_data.items():
    # Add a section header for the bars_club type
    doc.add_heading(f"Bars and Clubs Type: {bars_club_type}", level=1)

    # Iterate through the bars_clubs of the current type
...
doc.save('bars_clubs_grouped.docx')
